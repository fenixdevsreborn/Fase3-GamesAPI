...
/// <summary>Meter facade for Games API: HTTP + games + library + recommendations + exceptions.</summary>
public sealed class FcgMeters
{
    ...
    private readonly Counter<long> _gamesUpdated;
    private readonly Counter<long> _gamesDeleted;
    private readonly Counter<long> _libraryItemsAdded;
    private readonly Counter<long> _libraryItemsRemoved;
    private readonly Counter<long> _recommendationsGenerated;
    ...
    public FcgMeters(string meterName)
    {
        ...
        _gamesCreated = _meter.CreateCounter<long>(FcgMetricNames.GamesCreated);
        _gamesUpdated = _meter.CreateCounter<long>(FcgMetricNames.GamesUpdated);
        _gamesDeleted = _meter.CreateCounter<long>(FcgMetricNames.GamesDeleted);
        _libraryItemsAdded = _meter.CreateCounter<long>(FcgMetricNames.LibraryItemsAdded);
        _libraryItemsRemoved = _meter.CreateCounter<long>(FcgMetricNames.LibraryItemsRemoved);
        _recommendationsGenerated = _meter.CreateCounter<long>(FcgMetricNames.RecommendationsGenerated);
    ...
    public void RecordGameUpdated() => _gamesUpdated.Add(1);
    public void RecordGameDeleted() => _gamesDeleted.Add(1);
    public void RecordLibraryItemAdded() => _libraryItemsAdded.Add(1);
    public void RecordLibraryItemRemoved() => _libraryItemsRemoved.Add(1);
    public void RecordRecommendationsGenerated(int count = 1) => _recommendationsGenerated.Add(count);
